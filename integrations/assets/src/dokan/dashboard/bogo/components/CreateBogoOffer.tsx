...
import { SimpleInput, SimpleRadio, TextArea, SearchableSelect, useToast, DokanToaster } from '@getdokan/dokan-ui';
// @ts-ignore
...
const CreateBogoOffer = ( { navigate, params } ) => {
    ...
    useEffect( () => {
        setFormData( ( prev ) => ( {
            ...prev,
            offer_schedule: selectedSchedules.map( ( schedule ) => schedule.value ).join( ',' )
        } ) );
    }, [selectedSchedules] );
