...
const Preview = ({ storeData }) => {
    ...
    const product_image_url = bogo_products_and_categories?.product_list_for_view[storeData.get_different_product_field]?.image_url;
    const addCommas = (number) => {
        return number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    }
    const product = bogo_products_and_categories?.product_list?.simpleProductForOffer
    ...
    if (storeData?.offer_type === 'discount') {
        const discountPercent = (parseFloat(storeData?.discount_amount + '%') / 100);
    ...
    return (
        <div
            ...
            className='bogo-preview-wrapper'
        >
            <div
            ...
            >
                ...
                { storeData?.product_page_message ?? '' }
            </div>
            <div
                className={`bogo-product-preview`}
                style={{
                    gap: 14,
                    display: 'flex',
                    alignItems: 'center',
            ...
            >
                ...
                >
                    ...
                    >
                        {typeof product !== 'undefined' && storeData?.offer_product_title ? storeData?.offer_product_title : __('Nike Air Max Plus', 'storegrowth-sales-booster')}
                    </Title>
                    <p
                ...
                >
                    {currentSettings?.regular_price_show && (
                        <div
                            style={{
                                marginRight: 16,
                    ...
                    {discountedPrice && ! isNaN( discountedPrice ) ? (product?.currency + addCommas(discountedPrice)) : __('$87.00', 'storegrowth-sales-booster')}
                </div>
            </div>
        </div>
    );
