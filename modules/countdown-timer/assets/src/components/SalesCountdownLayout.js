...
function SalesCountdownLayout({ navigate, useSearchParams, moduleId }) {
  ...
  // apply filters to allow other modules to modify the initial data.
    const filteredInitialData = applyFilters( 'spsg_countdown_timer_initial_data', initialSalesCountdownData );
  ...
  const onFormSave = (type) => {
    ...
    let data = {
      action: "spsg_countdown_timer_save_settings",
  ...
  const getSettings = () => {
    ...
    jQuery
      .ajax({
        ...
        method: "POST",
        data: {
          action: "spsg_countdown_timer_get_settings",
          _ajax_nonce: spsgAdmin.nonce,
        },
      })
      .then((response) => {
