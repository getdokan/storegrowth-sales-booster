...
const { CheckboxControl } = wc.blocksComponents;

/**
 * OrderBumpTemplate Component
 *
 * @param {Object} props
 * @param {Object} props.design - Styling and configuration data for the bump box.
 * @param {Object} props.offerData - Offer-related data (type, price, etc.).
 */
const OrderBumpTemplate = ({design, offerData}) => {
    const {
        box_border_style,
        box_border_color,
    ...
    const handleChange = () => {
        ...
        window.jQuery.post(window.bump_save_url.ajax_url_for_front, {
    ...
    const imageUrl =
        offer_image_url && offer_image_url !== "http://false"
            ? offer_image_url
            : fallback_image_url;

    const borderStyle =
        box_border_style !== "no_border"
            ? `2px ${box_border_style} ${box_border_color}`
            : "none";

    const offerText =
        offer_type === "discount"
            ? `${offer_amount}${offer_discount_title}`
            : `${offer_amount}.00${offer_fixed_price_title}`;

    return (
        <div className="template-overview-area" style={{paddingLeft: '16px', paddingRight: '16px'}}>
            <div
            ...
            >
                ...
                <div
                    ...
                    style={{
                        background: discount_background_color,
                        color: discount_text_color,
                        fontSize: `${discount_font_size}px`,
                    }}
                >
                    <svg
                        width="15"
                ...
                <div className="product-image-and-title">
                    <div className="offer-product-image-title">
                        <div className="offer-product-image">
                            <img src={imageUrl} width="70" alt={offer_product_title}/>
                        </div>
                        <div
                            className="offer-product-title"
                            style={{
                                color: product_description_text_color,
                                fontSize: `${product_description_font_size}px`,
                            }}
                        >
                            <h3
                                style={{
                            ...
                            {category_names?.length > 0 && (
                                <p style={{color: product_description_text_color}}>
                                    {category_names.join(", ")}
                                </p>
                            )}
                        </div>
...
export default OrderBumpTemplate;
