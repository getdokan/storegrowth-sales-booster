...
class Enqueue_Script {
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( 'sales-booster_page_spsb-settings' === $hook ) {
			...
			$settings_file                   = require spsb_modules_path( 'upsell-order-bump/assets/build/settings.asset.php' );
			$settings_file['dependencies'][] = 'jquery';

			wp_enqueue_script(
				'spsb-order-bump-settings',
				spsb_modules_url( 'upsell-order-bump/assets/build/settings.js' ),
			...
			wp_localize_script(
				...
				array(
					...
					'ajd_nonce'    => $ajd_nonce,
					'image_folder' => spsb_modules_url( 'upsell-order-bump/assets/images' ),
				)
			);
		}
	}
	...
	public function admin_enqueue_styles() {
		$ftime          = filemtime( spsb_modules_path( 'upsell-order-bump/assets/css/order-bump-custom-admin.css' ) );
		$ftime_template = filemtime( spsb_modules_path( 'upsell-order-bump/assets/css/order-bump-template.css' ) );

		wp_enqueue_style(
			'spsb-order-bump-custom-admin-css',
			spsb_modules_url( 'upsell-order-bump/assets/css/order-bump-custom-admin.css' ),
			null,
			$ftime
		);

		wp_enqueue_style(
			'spsb-order-bump-template-css',
			spsb_modules_url( 'upsell-order-bump/assets/css/order-bump-template.css' ),
			null,
			$ftime_template
		);
	}
	...
	public function front_styles() {
		$ftime = filemtime( spsb_modules_path( 'upsell-order-bump/assets/css/order-bump-front.css' ) );

		wp_enqueue_style(
			'spsb-order-bump-front-css',
			spsb_modules_url( 'upsell-order-bump/assets/css/order-bump-front.css' ),
			null,
			$ftime
		);
	}
	...
	public function front_scripts() {
		$ftime = filemtime( spsb_modules_path( 'upsell-order-bump/assets/js/order-bump-custom.js' ) );

		wp_enqueue_script(
			'spsb-order-bump-front-js',
			spsb_modules_url( 'upsell-order-bump/assets/js/order-bump-custom.js' ),
			'jquery',
			$ftime,
			true
		);
