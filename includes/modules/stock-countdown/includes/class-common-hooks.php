...
class Common_Hooks {
	...
	public function woocommerce_product_data_tabs( $tabs ) {
		...
		$tabs['stock_countdown_tab'] = array(
			...
			'target' => 'spsb-stock-countdown-tab',
		);
	...
	public function woocommerce_admin_process_product_object( $product ) {
		...
		if ( isset( $_POST['_spsb_stock_countdown_discount_start'] ) ) {
			$discount_start_date = wc_clean( wp_unslash( $_POST['_spsb_stock_countdown_discount_start'] ) ); //phpcs:ignore
		...
		if ( isset( $_POST['_spsb_stock_countdown_discount_end'] ) ) {
			$discount_end_date = wc_clean( wp_unslash( $_POST['_spsb_stock_countdown_discount_end'] ) ); // phpcs:ignore
		...
		$stock_discount_amount = isset( $_POST['_spsb_stock_countdown_discount_amount'] ) ? wc_clean( wp_unslash( $_POST['_spsb_stock_countdown_discount_amount'] ) ) : null; // phpcs:ignore

		update_post_meta( $product->get_id(), '_spsb_stock_countdown_discount_start', $discount_start_date );
		update_post_meta( $product->get_id(), '_spsb_stock_countdown_discount_end', $discount_end_date );
		update_post_meta( $product->get_id(), '_spsb_stock_countdown_discount_amount', $stock_discount_amount );
