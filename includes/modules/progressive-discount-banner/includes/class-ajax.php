...
class Ajax {
	...
	private function __construct() {
		add_action( 'wp_ajax_spsb_pd_banner_save_settings', array( $this, 'save_settings' ) );
		add_action( 'wp_ajax_spsb_pd_banner_get_settings', array( $this, 'get_settings' ) );
	}
	...
	public function save_settings() {
		...
		foreach ( $_POST['form_data'] as $form_key => $form_value ) {
			if ( in_array( $form_key, $icon_validator, true ) ) {
				$form_data[ $form_key ] = spsb_sanitize_svg_icon_fields( $form_value );
			} else {
				$form_data[ $form_key ] = spsb_sanitize_form_fields( $form_value );
			}
		}

		$get_form_data = spsb_pd_banner_get_settings();
		$merged_data   = array_merge( $get_form_data, $form_data );

		update_option( 'spsb_progressive_discount_banner_settings', $merged_data );
	...
	public function get_settings() {
		...
		wp_send_json_success( spsb_pd_banner_get_settings() );
	}
