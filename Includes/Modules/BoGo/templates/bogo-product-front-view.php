...
<div class='template-overview-area'>
		<hr style="margin-top:<?php echo esc_attr( $bogo_info->box_top_margin ); ?>px"/>
		<div class="offer-main-wrap" 
		style="<?php echo 'no_border' !== $bogo_info->box_border_style ? 'border:2px ' . esc_attr( $bogo_info->box_border_style ) . ' ' . esc_attr( $bogo_info->box_border_color ) : ''; ?>">
			<div class="dynamic-offer-text"
			style="
			<?php
			echo 'background:' . esc_attr( $bogo_info->discount_background_color ) . ';';
			echo 'color:' . esc_attr( $bogo_info->discount_text_color ) . ';';
			echo 'font-size:' . esc_attr( $bogo_info->discount_font_size ) . 'px;'
			?>
			"
			>
			...
			<?php
			echo 'discount' === $offer_type ? '&nbsp;' . esc_attr( $offer_amount . $bogo_info->offer_discount_title ) : esc_attr( $offer_amount ) . '.00' . esc_attr( $bogo_info->offer_fixed_price_title );
			$fallback_image_url = esc_url( plugin_dir_url( __FILE__ ) . '../assets/images/bump-preview.svg' );
			$image_url          = 'http://false' !== $bogo_info->offer_image_url ? $bogo_info->offer_image_url : $fallback_image_url;
			?>
			</div>
			<div class="product-image-and-title">
				...
				style = "
				<?php
				echo 'color:' . esc_attr( $bogo_info->product_description_text_color ) . ';';
				echo 'font-size:' . esc_attr( $bogo_info->product_description_font_size ) . 'px;'
				?>
				"
				>
					<h3 style="color:<?php echo esc_attr( $bogo_info->product_description_text_color ); ?>">
					<?php echo esc_attr( $bogo_info->offer_product_title ); ?>
					</h3>

					<?php
					// Collect offer product categories.
					$product_id         = $bogo_info->offer_product;
